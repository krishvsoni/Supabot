...
console.log('Enhanced Database Migration Script\n');
...
async function runMigration() {
  ...
  try {
    ...
    if (error && error.code === '42P01') {
      console.log('semantic_search_view not found - needs creation');
    } else {
      console.log('semantic_search_view exists');
    }
  } catch (error) {
    console.log('Could not check semantic_search_view');
  }
  ...
  try {
    ...
    if (error && error.code === '42P01') {
      console.log('chat_logs table not found - needs creation');
    } else {
      console.log('chat_logs table exists');
    }
  } catch (error) {
    console.log('Could not check chat_logs table');
  }
  ...
  try {
    ...
    if (error) {
      ...
      if (data && data.length > 0) {
        ...
        console.log('No documents found - you may need to run the embedding process');
      }
    }
  } catch (error) {
